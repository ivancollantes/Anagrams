...
object Anagrams {
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    val occurrencesList: List[(Occurrences, Word)] = dictionary.map((w: Word) => (wordOccurrences(w), w))
    occurrencesList groupBy { pair => pair._1 } mapValues { list => list map { pair => pair._2} }
//    occurrencesList groupBy { _._1 } mapValues { _ map { _._2} } // shorter version
  }
